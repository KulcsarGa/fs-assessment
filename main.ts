...
const writeFilesHashed = async () => {
  const fs = new FS("./topdir");
  await fs.store("filename1", "a very long string1");
  await fs.store("filename2", "a very long string1");
  await fs.store("filename3", "a very long string3");
